...
from data_generator import OMAPS2Dataset, Augmentor, OMAPS2Sampler, OMAPS2TestSampler, collate_fn
...
def train(args):
    ...
    # Paths
    hdf5s_dir = os.path.join(workspace, 'hdf5s', 'omaps2')
    ...
    # Dataset
    train_dir = os.path.join(hdf5s_dir, 'train')
    valid_dir = os.path.join(hdf5s_dir, 'valid')
    test_dir = os.path.join(hdf5s_dir, 'test')
    train_dataset = OMAPS2Dataset(hdf5s_dir=train_dir, 
        segment_seconds=segment_seconds, frames_per_second=frames_per_second, 
        max_note_shift=max_note_shift, augmentor=augmentor)

    evaluate_dataset = OMAPS2Dataset(hdf5s_dir=valid_dir, 
        segment_seconds=segment_seconds, frames_per_second=frames_per_second, 
        max_note_shift=0)

    evaluate_train_dataset = OMAPS2Dataset(hdf5s_dir=train_dir, 
        segment_seconds=segment_seconds, frames_per_second=frames_per_second, 
        max_note_shift=0)

    test_dataset = OMAPS2Dataset(hdf5s_dir=test_dir, 
        segment_seconds=segment_seconds, frames_per_second=frames_per_second, 
        max_note_shift=0)

    # OMAPS2Sampler for training
    train_sampler = OMAPS2Sampler(hdf5s_dir=train_dir, 
        segment_seconds=segment_seconds, hop_seconds=hop_seconds, 
        batch_size=batch_size, mini_data=mini_data)

    # OMAPS2Sampler for evaluation
    evaluate_train_sampler = OMAPS2TestSampler(hdf5s_dir=train_dir, 
        segment_seconds=segment_seconds, hop_seconds=hop_seconds, 
        batch_size=batch_size, mini_data=mini_data)

    evaluate_validate_sampler = OMAPS2TestSampler(hdf5s_dir=valid_dir, 
        segment_seconds=segment_seconds, hop_seconds=hop_seconds, 
        batch_size=batch_size, mini_data=mini_data)

    evaluate_test_sampler = OMAPS2TestSampler(hdf5s_dir=test_dir, 
        segment_seconds=segment_seconds, hop_seconds=hop_seconds, 
        batch_size=batch_size, mini_data=mini_data)
    ...
    evaluate_train_loader = torch.utils.data.DataLoader(dataset=evaluate_train_dataset, 
